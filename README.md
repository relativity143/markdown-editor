...
- **三种编辑模式**
  ...
  - 所见即所得（IR，源码与渲染混排）
  - 分屏预览（SV，左源码 / 右预览）
- **公式块**：行内 `$E=mc^2$`、块级 `$$ ... $$`，使用 KaTeX 渲染
- **代码高亮**、表格、任务列表、引用、脚注、Emoji
- **Mermaid / 流程图 / 时序图**（由 Vditor 提供）
- **本地文件读写**：新建 / 打开 / 保存 / 另存为（优先使用 File System Access API，可直接覆盖原文件）
- **导出**：HTML 文件 / 通过浏览器打印为 PDF
- **自动保存**到浏览器本地存储，下次打开自动恢复
- **大纲**：根据 `#` 标题自动生成，点击跳转
- **明 / 暗主题**、打字机模式、字数 / 字符统计
...
- `dist/mac-arm64/Markdown Editor.app` — 直接双击运行的 App
- `dist/Markdown Editor-1.0.0-arm64.dmg` — 拖拽到 Applications 的安装镜像
- `dist/Markdown Editor-1.0.0-arm64-mac.zip` — 便携 zip
...
├── app.js                  # 渲染进程（编辑器、菜单、文件、大纲、主题）
├── electron/
│   ├── main.js             # 主进程（窗口、原生菜单、IPC、文件关联）
│   └── preload.js          # 安全桥（contextBridge）
├── assets/
│   └── icon.svg            # 图标源
├── scripts/
│   ├── make-icon.js        # SVG → .icns
│   └── build-with-proxy.sh # 走代理打包
├── package.json
└── README.md
```
